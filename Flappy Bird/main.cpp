...
int main() {
	...
	sounds.hopBuffer.loadFromFile("./audio/swoosh.wav");
	sounds.dishkBuffer.loadFromFile("./audio/die.wav");
	sounds.ching.setBuffer(sounds.chingBuffer);
	sounds.hop.setBuffer(sounds.hopBuffer);
	sounds.dishk.setBuffer(sounds.dishkBuffer);

	// all textures remain in here, the bird have 3 images which randomly changes by which it looks like flying.
	struct Textures {
		Texture flappy[3];
		Texture pipe;
		Texture background;
	...
	game.pressSpace.setString("Press Space to continue");
	game.pressSpace.setFont(game.font);
	game.pressSpace.setFillColor(Color::White);
	game.pressSpace.setCharacterSize(50);
	game.pressSpace.setOrigin(game.pressSpace.getLocalBounds().width / 2, 0);
	game.pressSpace.setPosition(500, 250);
	...
	while (window.isOpen()) {
		...
		while (window.pollEvent(event)) {
			...
			// flap event i.e. mouse right click should be pressed to move the bird upwards
		...
		// gameover. press space to continue
